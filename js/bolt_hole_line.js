...
data={
    ...
    function init(fields_) {
		...
		try{
		var ret;
 	     ret=window.localStorage.getItem('bolt_hole_line_last_state');
		 if (ret!== null){
		    data=JSON.parse(ret);
        }else{
           data.rnd=3;
...
function Calc(control){
    current_parent="panel_bolt_hole_line_calc";
    editElementByName("clear","innerHTML","");

    data.scale=parseFloat(Get_Value(fields.scale));

    data.length=parseFloat(Get_Value(fields.length));
    data.spacing=parseFloat(Get_Value(fields.spacing));
    data.distance=parseFloat(Get_Value(fields.distance));
    data.holes=parseFloat(Get_Value(fields.holes));

    if (typeof(control)!="undefined"){
        if (control.id==fields.length){
            data.spacing=data.length/(data.holes-1)
        }
        if (control.id==fields.spacing || control.id==fields.holes ){
...
//display results here
    ...
    try{
        localStorage.setItem('bolt_hole_line_last_state', JSON.stringify(data));
    }catch(e){return}
...
function draw_dim(control){
   ...
   Drw.dim2point(data.x[0]-data.xc, data.y[0]-data.yc, x1-data.xc, y1-data.yc,20/data.scale_); // co-ordinates of line 1
    }

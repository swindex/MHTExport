function bolt_hole_partial_calc(){
var data= new Object();
var curr_dims;
var Drw;
var fields={};
...
function init(fields_) {
		...
		try{
		var ret=null;
 	     ret=window.localStorage.getItem('bolt_hole_partial_calc_last_state');
		 if (ret!== null){
		    data=JSON.parse(ret);
        }else{
           data.rnd=3;
		...
		Set_Value(fields.radius,data.radius);
		Set_Value(fields.angle,data.angle);
		Set_Value(fields.angle_between,data.angle_between);
...
if (data.radius<=0) {editElementById("error_"+fields.radius,"innerHTML","radius must be bigger than 0");return;}

if (lic_type!="pro" && data.radius > 10 ){
	editElementById("error_"+fields.radius,"innerHTML","Please buy PRO version to unlock");
	data.radius=10
}
...
function draw_dim(control){
        ...
        bhc.setColor("green"); //
        if (control.id==fields.radius){
            //x1=data.x[data.x.length-1];
            //y1=data.y[data.y.length-1];
            Drw.dim2point(0,0, data.x[0], data.y[0],20/data.scale_); // co-ordinates of line 1
        }
        if (control.id==fields.angle){
            x1=data.x[0]//-data.xc;
            y1=data.y[0]//-data.yc;
            bhc.drawLine(0, 0, x1,y1); // co-ordinates of line 1
            bhc.drawLine(0, 0,data.radius, 0); // co-ordinates of line 1
            if (data.angle>1){
                Drw.dimangle(0, 0,40/data.scale_, 20/data.scale_,0,data.angle);
            }
        }
        if (control.id==fields.angle_between && data.x.length > 1){
            x1=data.x[0]//-data.xc;
            y1=data.y[0]//-data.yc;
            bhc.drawLine(0, 0, x1,y1); // co-ordinates of line 1
            bhc.drawLine(0, 0,data.x[1], data.y[1]); // co-ordinates of line 1
            if (data.angle_between>1){
                Drw.dimangle(0, 0,40/data.scale_, 20/data.scale_,data.angle,data.angle_between);
            }
        }
...
data.scale_=(data.canvas_width/f_.radius/2*.75)//*data.scale;
...
var d_angle=deg2rad(f_.angle_between)
...
f_.y=Array()
  ...
  for (i=0;i<f_.holes;i++){
	...
	x =Math.cos(angle+d_angle*(i))*f_.radius;
	y =Math.sin(angle+d_angle*(i))*f_.radius;
	x1 =f_.xc+x;
	y1 =f_.yc+y;
